...
- PCブラウザ・Androidブラウザから、USBで接続したNanapoへテキストボックスの文字列（最大40文字）を送信し、7セグディスプレイに表示させる
- 表示はスクロールアニメーション形式。8文字以内でも右から登場して左へ退場し、8文字を超える文字列は左向きに流れ続けるマーキー表示になる
...
3. テキストボックスに表示したい文字列（英数字・記号、最大40文字）を入力し、「送信」ボタン（またはEnterキー）で送信する。8文字を超える場合や超えない場合を問わず、常にスクロールアニメーションで表示される。アニメーション中に「停止」ボタンを押すか、別の文字列・コマンドを送信すると、その時点でアニメーションは打ち切られる
...
- 通信速度: 115200bps
- 改行コード: CRLF (`\r\n`)
- Nanapo自体は一度に最大8文字までしか表示できません。本アプリはこの制約を踏まえ、入力文字列の前後に8文字分の空白を付加してから8文字幅の窓を1文字ずつスライドさせ、生成した各フレーム（8文字の文字列）を約250msごとに送信することでスクロール表示を実現しています（8文字以内の文字列は中央で約1.5秒静止してから退場します）
