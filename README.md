...
3. 「スクロール表示」（最大40文字）か「通常表示」（最大8文字）を選び、テキストボックスに表示したい文字列を入力して「送信」ボタン（またはEnterキー）で送信する
   - スクロール表示は停止するまで無限に繰り返す。「停止」ボタンを押すか、別の文字列・コマンドを送信すると、その時点でアニメーションは打ち切られる（停止ボタンは表示を`@CLR`でクリアする）
