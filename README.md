...
- 通信速度: 115200bps
- 改行コード: CRLF (`\r\n`)
- Nanapo自体は一度に最大8文字までしか表示できません。スクロール表示はこの制約を踏まえ、入力文字列の前後に8文字分の空白を付加してから8文字幅の窓を1文字ずつスライドさせ、生成した各フレーム（8文字の文字列）を約250msごとに送信することで実現しています（8文字以内の文字列は右詰めの状態で約1.5秒静止してから退場し、これを停止するまで繰り返します）
